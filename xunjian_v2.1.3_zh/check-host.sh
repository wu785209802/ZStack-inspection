...
function xms_cpu_check(){
    echo "###################################xmsd cpu check###################################"
    ceph version >/dev/null 2>&1
    result=`echo $?`
    top_str=`top -b -n 1`
    if [[ $result -eq 0 ]];then
        xsmd_cpu=`echo "$top_str" | grep "xmsd" | awk -F " " '{print $9}'`
        echo "$xsmd_cpu" | awk '{if ($0 > 100){print "Urgent: xmsd 的使用率超过100" } else { print "Normal: xmsd 的使用 率低于100" }}'
    fi
}
...
function getSanInfo(){
    ...
    echo ""
    echo "发生异常的的链路:"
    multipath -ll | grep failed  | awk -F " " '{if ($5=="failed") {print  $0" ""Urgent: 此链路存在故障，请检查"}}'
}

#function getCephInfo(){
#
#    # 检查是否为Ceph类型物理机，若是，继续检查；若不是，则退出该函数
#    ceph -s >> /dev/null 2>&1
#    if [ $? -ne 0 ];then
#	return;
#    else
#	echo "该环境为Ceph超融合部署"
#	swapGB=$(free -g | awk 'NR==3{print $2}')
#	rootCapacity=$(df -lBG | awk '{if($6=="/") print $4}' | sed 's/G//g')
#	if [ ${swapGB} -ne 0 ];then
#	    echo "Urgent：物理机SWAP未关闭！请联系ZStack技术支持工程师协助关闭！"
#	fi
#	if [ ${rootCapacity} -lt 480 ];then
#		echo "Warning：物理机根分区小于480GB！"
#	fi
#    fi
#    echo "############################ Ceph信息检查 #############################"
#
#    # Ceph 硬盘信息检查
#
#    check_tmp=/tmp/host_mon_check.log
#    rm -f $check_tmp
#    ip addr|grep inet|grep -v 127.0.0.1|grep -v inet6|awk '{print $2}'|sed 's/\/.*$//'>$check_tmp
#    ceph mon dump|awk -F : '/mon/ {print $2}'|sed 's/ //g' >>$check_tmp
#    Host_Mon_NUM=`cat $check_tmp |sort | uniq -c| sort -n -r|head -1|awk '{print $1}'`
#    #rm -f $check_tmp
#
#    echo
#    echo "Ceph OSD List on Host"
#    df -lh|grep osd|awk '{print $NF}'|sed 's/\/var\/lib\/ceph\/osd\///g'
#
#    if [ $Host_Mon_NUM -ge 2 ]; then
#        echo
