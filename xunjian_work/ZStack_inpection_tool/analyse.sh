...
EOF
    ...
    echo "物理机的主机名信息:"
    grep "主机名" $Total_Host_Log -ir| awk -F "/|:" '{print $5,$8}' | column -t
    echo

    echo "物理机CPU C State信息:"
    grep "CPU C state"  $Total_Host_Log -r | awk -F "txt:" '{print $NF}'
    echo

    echo "物理机CPU的温度信息:"
    grep "CPU temperature"  $Total_Host_Log -r | awk -F "txt:" '{print $NF}'
    echo

    echo "物理机的CPU核心数:"
    grep "CPU核心数: "  $Total_Host_Log -ir | grep -v "每" | awk -F "/|:" '{print $5 ,$7 ,$8}' | column -t
    echo

    echo "物理机已开机运行时间:"
    grep "运行时间" $Total_Host_Log -ir| awk -F "/|:" '{print $5 ,$7 ,$8}' |column -t
    echo

    echo "物理机上的进程数量信息:"
    grep "进程数量" $Total_Host_Log -ir| awk -F "/|:" '{print $5 ,$7 ,$8}' |column -t
    echo

    echo "物理机上系统负载信息:"
    grep "系统负载: " $Total_Host_Log -ir| awk -F "/|:" '{print $5 ,$7 ,$8}' |column -t
    echo

    echo "物理机上已用内存信息:"
    grep "已用内存"  $Total_Host_Log -ir|awk -F "/|:" '{print $5,$7,$8,$9,$10}' |column -t
    echo
    ...
    echo "物理机上系统盘使用信息:"
    grep "系统盘使用"  $Total_Host_Log -ir|awk -F "/|:" '{print $5,$7,$8,$9,$10}' | column -t
    echo
    ...
    echo "物理机上内存RSS使用信息:"
    grep "System Memory RSS Info:" $Total_Host_Log -irA 1| grep -v "Info" | grep "txt" | awk -F "/|txt-" '{print $5,$NF}' | column -t
    echo
    ...
    grep "磁盘检查" $Total_Host_Log -ir -A 50|grep  "|"|grep -v Mounted |awk -F '[ %]+' '{ if ($7>=30) print}'|awk '{print $1, $4,$5,$7,$NF}'|awk '{gsub(/\/tmp\/inspectionlog-[^|]*log\//,"" ); print}'|awk '{gsub(/\/HostDailyCheck-[^|]*txt-/," " ); print}'
    echo
    ...
    grep "Bonding Mode Slave Info:" $Total_Host_Log -ir -A 20|grep Bond_Info|awk '{gsub(/\/HostDailyCheck-[^|]*txt-/," " ); print}'|awk '{gsub(/\/tmp\/inspectionlog-[^|]*\//,"" ); print}'|sed 's/log\///g'|sed 's/Bond_Info|//g'
    echo

    echo "物理机上网卡、网卡是否链接、网卡驱动、网卡带宽、网卡设备型号信息，请仔细确认核对:"
    grep "device  link_status  driver  speed  vendor_device" $Total_Host_Log -A 50 -ir|egrep "Ethernet|bonding"|awk '{gsub(/\/tmp\/inspectionlog-[^|]*log\//,"" ); print}'|awk '{gsub(/\/HostDailyCheck-[^|]*txt-/," " ); print}'
    echo
