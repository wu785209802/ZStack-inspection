...
function cephEmail(){
    ...
    NAME=$user
    PASSWORD=$password
    token_id=`curl -s POST "http://$IP:8056/v1/auth/tokens" -H "accept: application/json" -H "Content-Type: application/json" -d "{\"auth\":{\"identity\":{\"token\":{\"uuid\":\"string\"},\"password\":{\"user\":{\"id\":0,\"password\":\"${PASSWORD}\",\"email\":\"string\",\"name\":\"${NAME}\"}}}}}" | $PARSE_JSON | grep -E "token\"\,\"uuid" | awk -F '"' '{print $6}'`
    status_str=`curl -s GET "http://$IP:8056/v1/emails/config" -H "accept: application/json" -H "Xms-Auth-Token:$token_id" | $PARSE_JSON`
...
function secretlist(){
    ...
    for ip in `xms-cli --user admin --password ${password} service list | egrep -v 'ID|+--' | awk '{print $14}' | sort|uniq`
        do echo $ip ;ssh -i /usr/local/zstack/apache-tomcat/webapps/zstack/WEB-INF/classes/ansible/rsaKeys/id_rsa -o StrictHostKeyChecking=no $ip virsh secret-list
    done
    echo ""
}
# 配置信息
