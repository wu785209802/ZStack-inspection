...
my($version) = '1.8.6';
my($timeout) = 120;
my($email) = 'songtao.liu@zstack.io';
...
sub df {
    ...
    if ($os_type eq "linux") {
        open($in, "df -lP -k -l 2> /dev/null |") || warn "exec df: $!";
        $debug_storage .= ">> df -lP -k -l\n";
    } else {
        open($in, "df -lk -t ufs 2> /dev/null |") || warn "exec df: $!";
        $debug_storage .= ">> df -lk -t ufs\n";
    }
...
sub email {
    ...
    print $out "To: $email\n";
    print $out "From: songtao.liu\@zstack.io\n";
