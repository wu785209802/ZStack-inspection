...
function ZS_Overview(){
	...
	echo
        ...
        grep "CPU C state"  $Total_Host_Log -r | awk -F "/|:" '{print $5":"$8}'
        echo
	...
	EFI_NUM=`grep "/boot/efi" $Total_Host_Log -ir |grep uuid -i|wc -l`
		if [ X$EFI_NUM == X$Total_Host_Num ];then
...
function OS_Check(){
    ...
    if [ $CT73_NUM -ne 0 ] || [ $CT75_NUM -ne 0 ] || [ $CT76_NUM -ne 0 ] ;then
    ...
    if [ X$CT74_NUM != X$Total_Host_Num ];then
        ...
        grep  "CentOS Linux release 7.4.1708 (Core)" $Total_Host_Log -ir |grep "发行版本："|awk -F "/|:" '{print $2,$4}'
        echo "There are $CT72_NUM Host using CentOS 7.2"
        grep  "CentOS Linux release 7.2.1511 (Core)" $Total_Host_Log -ir |grep "发行版本："|awk -F "/|:" '{print $2,$4}'
    fi
    if [ X$CT74_NUM == X$Total_Host_Num ];then
...
function Kernel_Check(){
    ...
    if [ $CT73_Kernel_NUM -ne 0 ] || [ $CT75_Kernel_NUM -ne 0 ] || [ $CT76_Kernel_NUM -ne 0 ] ;then
        echo "Warning: There are hosts using the Kernel not supported by ZStack!"
    fi
    if [ X$CT74_Kernel_NUM != X$Total_Host_Num ];then
        echo "Warning: There aren't all hosts using the same system!"
        echo "There are $CT74_Kernel_NUM Host using the CentOS 7.4.1708 3.10.0-693"
